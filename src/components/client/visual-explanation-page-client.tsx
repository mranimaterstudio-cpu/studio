...
import { generateImageWithExplanation } from '@/ai/flows/generate-image-with-explanation';
import { z } from 'zod';
...
export function VisualExplanationPageClient() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const generationResult = await generateImageWithExplanation(prompt);
          ...
          <form onSubmit={handlePromptSubmit} className="space-y-4">
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Enter a concept and the AI will generate an explanation with an image.
              </p>
            </div>
             ...
             <div className="space-y-4 w-full max-w-2xl mx-auto">
                {result.imageUrl && (
                    <div className="relative aspect-video w-full rounded-md overflow-hidden border">
                         <Image src={result.imageUrl} alt={result.explanation.title} fill className="object-contain" />
                    </div>
                )}
                <div className='space-y-2'>
